    ...
    <AdminLayout>
        <template #header>
            <div class="flex justify-between items-center">
                <h2 class="font-semibold text-xl text-brand-text leading-tight">制約編集</h2>
        ...
        <div class="py-12">
            <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            ...
                            <form @submit.prevent="submit">
                                <div class="space-y-6">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-brand-text-muted mb-2">規約説明者の選択元となる店舗を選択（複数可）</p>
                                        <div class="flex flex-wrap gap-3">
                                            <label
                                                v-for="shop in shops"
                                                :key="shop.id"
                                                class="inline-flex items-center gap-2 px-3 py-2 rounded-lg border border-brand-border bg-brand-surface-2 text-sm cursor-pointer hover:bg-brand-surface-2"
                                            >
                                                <input
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-brand-text-muted mb-2">
                                            マークダウン形式で記入。チェック項目は <code class="bg-brand-surface-2 px-1 rounded">- [ ] ラベル</code> で記述してください。
                                        </p>
                                        <textarea
                                            v-model="form.body"
                                            rows="16"
                                    ...
                                    <div class="border-t border-brand-border pt-6">
                                        <h3 class="text-sm font-semibold text-brand-text mb-3">表示設定（印刷・プレビュー用）</h3>
                                        <p class="text-xs text-brand-text-muted mb-2">右側のプレビューでリアルタイムに確認できます。</p>
                                        <a
                                            :href="previewUrl"
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            class="inline-flex items-center gap-2 text-sm text-brand-primary hover:text-brand-primary-hover font-medium mb-4"
                                        >
                                        ...
                                        <div class="grid grid-cols-2 gap-4">
                                            <div>
                                                <label class="block text-xs font-medium text-brand-text-muted mb-1">余白（mm）</label>
                                                <input
                                            ...
                                            <div>
                                                <label class="block text-xs font-medium text-brand-text-muted mb-1">行間</label>
                                                <input
                                            ...
                                            <div>
                                                <label class="block text-xs font-medium text-brand-text-muted mb-1">フォントサイズ（px）</label>
                                                <input
                                            ...
                                            <div>
                                                <label class="block text-xs font-medium text-brand-text-muted mb-1">署名欄高さ（px）</label>
                                                <input
                                            ...
                                            <div>
                                                <label class="block text-xs font-medium text-brand-text-muted mb-1">チェックボックス（px）</label>
                                                <input
                                    ...
                                    <div>
                                        ...
                                        <p class="mt-1 text-xs text-brand-text-muted">無効にすると、顧客への制約追加で選択できなくなります。</p>
                    ...
                    <!-- 右: プレビュー -->
                    <div class="bg-brand-surface overflow-hidden shadow-sm sm:rounded-lg h-fit">
                        <div class="p-4 border-b border-brand-border bg-brand-surface-2">
                            <h3 class="text-sm font-semibold text-brand-text">プレビュー</h3>
                            <p class="text-xs text-brand-text-muted mt-0.5">印刷時の表示イメージです</p>
                        </div>
                        <div
                            class="p-4 overflow-auto max-h-[calc(100vh-280px)] preview-container"
                            :style="previewContainerStyle"
                        >
                            <div class="preview-inner bg-brand-surface border border-brand-border rounded-lg shadow-inner" :style="previewInnerStyle">
                                <div class="mb-2 pb-2 border-b border-brand-border">
                                    <p class="text-brand-text-muted" :style="{ fontSize: ds.font_size_px + 'px' }">
                                        顧客名: <span class="font-semibold text-brand-text">{{ form.name || '（制約名）' }}</span>
                                    </p>
                                </div>
                                <h2
                                    class="text-center font-bold text-brand-text mb-4"
                                ...
                                <div class="border-t border-brand-border mt-4 pt-4">
                                    <div class="grid grid-cols-2 gap-4 mb-4" :style="{ fontSize: ds.font_size_px + 'px' }">
                                        <div>
                                            <span class="text-brand-text-muted">日付：</span>
                                            <span class="font-semibold">____年____月____日</span>
                                        </div>
                                        <div>
                                            <span class="text-brand-text-muted">規約説明者：</span>
                                            <span class="font-semibold">________________</span>
                                        </div>
                                    </div>
                                    <div class="mb-2" :style="{ fontSize: ds.font_size_px + 'px' }">
                                        <span class="text-brand-text-muted">ご署名</span>
                                    </div>
                                    <div
                                        class="border-2 border-dashed border-brand-border rounded bg-brand-surface-2 flex items-center justify-center text-brand-text-subtle text-sm"
...
const form = useForm({
    ...
    display_settings: {
        ...
        ...(props.constraintTemplate.display_settings || {}),
    },
});
...
const previewUrl = computed(() => {
    const base = route('admin.constraint-templates.preview');
    const params = new URLSearchParams({
        template_id: props.constraintTemplate.id,
        padding_mm: ds.value.padding_mm,
        line_height: ds.value.line_height,
        font_size_px: ds.value.font_size_px,
        signature_height_px: ds.value.signature_height_px,
        checkbox_size_px: ds.value.checkbox_size_px,
    });
