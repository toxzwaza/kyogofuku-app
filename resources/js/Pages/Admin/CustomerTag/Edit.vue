    ...
    <AdminLayout>
        <template #header>
            <div class="flex justify-between items-center">
                <h2 class="font-semibold text-xl text-brand-text leading-tight">顧客タグ編集</h2>
                        ...
                        <form @submit.prevent="submit">
                            <div class="space-y-6">
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center space-x-4">
                                        <input
                                            v-model="form.color"
                                            type="color"
                                            class="h-10 w-20 rounded-md border-brand-border shadow-sm focus:border-brand-primary focus:ring-brand-primary cursor-pointer"
                                        />
                                        <input
                                            v-model="form.color"
                                            type="text"
                                    ...
                                    <p class="mt-1 text-xs text-brand-text-muted">タグの表示色を設定します（例: #FF5733）</p>
                                ...
                                <div>
                                    ...
                                    <p class="mt-1 text-xs text-brand-text-muted">無効にすると、このタグは使用できなくなります。</p>
...
const form = useForm({
    name: props.customerTag.name || '',
    description: props.customerTag.description || '',
    color: props.customerTag.color || '',
    is_active: props.customerTag.is_active ?? true,
});

const submit = () => {
    form.put(route('admin.customer-tags.update', props.customerTag.id));
};
</script>
