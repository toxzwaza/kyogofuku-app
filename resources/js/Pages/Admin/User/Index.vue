    ...
    <AdminLayout>
        <template #header>
            <div class="flex justify-between items-center">
                <h2 class="font-semibold text-xl text-brand-text leading-tight">スタッフ一覧</h2>
                ...
                <div class="bg-brand-surface overflow-hidden shadow-sm sm:rounded-lg mb-6">
                    <div class="p-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-brand-text mb-1">名前</label>
                                <input
                            ...
                            <div>
                                <label class="block text-sm font-medium text-brand-text mb-1">メールアドレス</label>
                                <input
                            ...
                            <table class="min-w-full divide-y divide-brand-border">
                                <thead class="bg-brand-surface-2">
                                    <tr>
                                        ...
                                        <th class="px-6 py-3 text-left text-xs font-medium text-brand-text-muted uppercase tracking-wider">名前</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-brand-text-muted uppercase tracking-wider">メールアドレス</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-brand-text-muted uppercase tracking-wider">テーマカラー</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-brand-text-muted uppercase tracking-wider">勤務属性</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-brand-text-muted uppercase tracking-wider">所属店舗</th>
                                ...
                                <tbody class="bg-brand-surface divide-y divide-brand-border">
                                    <tr v-for="user in users.data" :key="user.id">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-brand-text">{{ user.id }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-brand-text">{{ user.name }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-brand-text">{{ user.email }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-brand-text">
                                            <div v-if="user.theme_color" class="flex items-center space-x-2">
                                                <div
                                                    class="w-8 h-8 rounded border border-brand-border"
                                                    :style="{ backgroundColor: user.theme_color }"
                                                ></div>
                                                <span class="text-brand-text-muted">{{ user.theme_color }}</span>
                                            </div>
                                    ...
                                    <tr v-if="users.data.length === 0">
                                        <td colspan="7" class="px-6 py-8 text-center text-brand-text-muted">
                                            スタッフが見つかりませんでした
                                        </td>
                                    </tr>
                                </tbody>
