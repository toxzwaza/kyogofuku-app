                    ...
                    <div class="p-6">
                        <form @submit.prevent="submit">
                            <div class="space-y-4">
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">休憩の取り方 <span class="text-red-500">*</span></label>
                                    <select
                                    ...
                                    <p class="mt-1 text-sm text-gray-500">休憩が変動する人は「都度入力」、一定の人は「所定固定」を選びます</p>
                                    <div v-if="form.errors.break_mode" class="mt-1 text-sm text-red-600">{{ form.errors.break_mode }}</div>
                                </div>

                                <div v-if="form.break_mode === 'fixed'">
                                    <label class="block text-sm font-medium text-gray-700 mb-1">所定休憩（分） <span class="text-red-500">*</span></label>
                                    <input
                                        ...
                                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                    />
                                    <p class="mt-1 text-sm text-gray-500">例: 60 = 1時間 / 85 = 1時間25分 / 90 = 1時間30分 / 0 = 休憩なし<span v-if="breakHint">（{{ breakHint }}）</span></p>
...
const form = useForm({
    ...
    break_mode: props.user.break_mode || 'manual',
    scheduled_break_minutes: props.user.scheduled_break_minutes ?? null,
    password: '',
    password_confirmation: '',
    shop_ids: props.user.shops ? props.user.shops.map(shop => shop.id) : [],
    main_shop_id: props.main_shop_id || null,
});
