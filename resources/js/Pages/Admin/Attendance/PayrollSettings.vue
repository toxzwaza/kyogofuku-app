    ...
    <AdminLayout>
        <template #header>
            <div class="flex justify-between items-center">
                <h2 class="font-semibold text-xl text-brand-text leading-tight">給与計算閾値設定</h2>
                <div class="flex gap-3 text-sm">
                    <Link :href="route('admin.attendance.payroll-simulator.index')" class="text-brand-primary hover:text-brand-primary-hover">給与計算シミュレーター</Link>
        ...
        <div class="py-12">
            <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="bg-brand-surface overflow-hidden shadow-sm sm:rounded-lg p-6 space-y-8">
                    <form @submit.prevent="submit">
                        <section>
                            <h3 class="text-base font-semibold text-brand-text mb-2">始業（出勤）</h3>
                            <p class="text-sm text-brand-text-muted mb-4">
                                シフト開始より前の打刻の扱いと、給与用出勤時刻の丸め（単位への最近接）を設定します。
                            </p>
                            <div class="space-y-4">
                                <div>
                                    <label class="block text-sm font-medium text-brand-text mb-1">早出とみなす閾値（分）</label>
                                    <input
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-brand-text mb-1">始業・丸め単位（分）</label>
                                    <input
                        ...
                        <section class="pt-6 border-t border-brand-border">
                            <h3 class="text-base font-semibold text-brand-text mb-2">終業（残業）</h3>
                            <p class="text-sm text-brand-text-muted mb-4">
                                シフト終了以降に算出した残業（分）を、以下の単位で<strong>切り捨て</strong>ます（短い方へ丸め）。
                            </p>
                            <div class="space-y-4">
                                <div>
                                    <label class="block text-sm font-medium text-brand-text mb-1">残業・丸め単位（分）</label>
                                    <input
                                        ...
                                        class="w-full rounded-md border-brand-border shadow-sm"
                                    />
                                    <p class="mt-1 text-xs text-brand-text-muted">残業はこの単位で<strong>切り捨て</strong>ます（例: 単位15分 → 38分→30分、44分→30分、45分→45分）。</p>
                        ...
                        <section class="pt-6 border-t border-brand-border">
                            <h3 class="text-base font-semibold text-brand-text mb-2">閾値方式の適用開始日</h3>
                            <p class="text-sm text-brand-text-muted mb-4">
                                閾値方式（勤務属性の「残業閾値」で残業を判定する新方式）を<strong>この日以降の勤務分</strong>から適用します。それより前の勤怠は従来どおり（据え置き）です。
                            </p>
                            <div>
                                <label class="block text-sm font-medium text-brand-text mb-1">適用開始日</label>
                                <input
                                    v-model="form.threshold_effective_date"
                                    type="date"
                                    class="w-full rounded-md border-brand-border shadow-sm"
                                />
                                <p class="mt-1 text-xs text-brand-text-muted">未設定のときは全期間に閾値方式を適用します。給与の締め期間の切り替わりに合わせることを推奨します。</p>
