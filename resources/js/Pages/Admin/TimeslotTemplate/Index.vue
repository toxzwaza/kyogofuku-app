    ...
    <AdminLayout>
        <template #header>
            <div class="flex justify-between items-center">
                <h2 class="font-semibold text-xl text-brand-text leading-tight">予約枠テンプレート一覧</h2>
                    ...
                    <div class="p-6">
                        <div v-if="templates && templates.length > 0" class="space-y-6">
                            <div
                                ...
                                :key="template.id"
                                class="border border-brand-border rounded-lg p-6 hover:shadow-md transition-shadow"
                            >
                                <div class="flex justify-between items-start mb-4">
                                    <div>
                                        <h3 class="text-lg font-semibold text-brand-text">{{ template.name }}</h3>
                                ...
                                <div v-if="template.slots && template.slots.length > 0" class="mt-4">
                                    <h4 class="text-sm font-medium text-brand-text mb-2">時間枠（{{ template.slots.length }}件）</h4>
                                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                        <div
                                            v-for="slot in template.slots"
                                            :key="slot.id"
                                            class="bg-brand-surface-2 rounded-md p-3 border border-brand-border"
                                        >
                                            <div class="text-sm">
                                                <span class="font-medium text-brand-text">
                                                    {{ String(slot.hour).padStart(2, '0') }}:{{ String(slot.minute).padStart(2, '0') }}
                                                </span>
                                                <span class="text-brand-text-muted ml-2">
                                                    （{{ slot.capacity }}枠）
                                                </span>
                                            </div>
                                        </div>
                                ...
                                <div v-else class="text-sm text-brand-text-subtle mt-4">
                                    時間枠が登録されていません
                                </div>
                            </div>
                        </div>
