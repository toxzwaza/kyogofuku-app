                        ...
                        <div class="border-t border-brand-border pt-6 print:pt-2">
                            <div class="grid grid-cols-2 gap-6 mb-6 print:gap-2 print:mb-2">
                                <div>
                                    ...
                                    <p
                                        v-if="isPreviewMode"
                                        class="text-brand-text font-semibold"
                                        :style="dsFontStyle"
                                    ...
                                    <template v-else>
                                        <input
                                            ...
                                            type="date"
                                            class="w-full rounded-lg border-brand-border shadow-sm focus:border-brand-primary focus:ring-brand-primary text-sm print:hidden"
                                        />
                                        <p
                                            class="hidden print:block text-brand-text font-semibold"
                                            :style="dsFontStyle"
                                        >{{ formatDateJa(form.signed_at) }}</p>
                                    </template>
                                </div>
...
const deleteConstraint = () => {
    if (!confirm('この制約情報を削除してもよろしいですか？')) return;
    router.delete(route('admin.customers.constraints.destroy', { customer: props.customer.id, customerConstraint: props.editId }));
};
