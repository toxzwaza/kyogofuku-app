...
class LineMessagingService
{
    ...
    /**
     * Webhook の replyToken を使ってテキストを返信する
     *
     * push と異なり月間メッセージ数の上限を消費しない。
     * replyToken は受信から短時間・1回のみ有効。
     *
     * @throws \RuntimeException
     */
    public function replyTextToUser(string $replyToken, string $text): void
    {
        $token = (string) config('line.messaging.channel_access_token', '');
        if ($token === '') {
            throw new \RuntimeException('LINE Messaging API のチャネルアクセストークンが未設定です（.env の LINE_MESSAGING_CHANNEL_ACCESS_TOKEN）。');
        ...
        if (! $response->successful()) {
            ...
            throw new \RuntimeException('LINE への返信に失敗しました: HTTP '.$response->status());
        }
    }
